 ...
 *
 * Even endpoints in even instances in host delay registration.
 * Odd endpoints in odd instances in host delay registration.
...
static void ipc_ept_entry(void *arg0, void *dummy1, void *dummy2)
{
	...
	if (inst_id >= ARRAY_SIZE(instances_array)) {
		printk("IPC-service HOST [INST %s - ENDP %s] demo "
				"cannot start. Missing instance id\n",
				ept_data->label, ept_data->ept_cfg.name);
		return;
	}

	printk("IPC-service HOST [INST %s - ENDP %s] demo started\n",
		       ept_data->label, ept_data->ept_cfg.name);
	...
	if (should_delay_registration(inst_id, ept_data->ept_id)) {
		/*
		 * Wait 1 sec to give the opportunity to the REMOTE core to register
		 * the endpoint first
		 */
	...
	while (message < 100) {
		...
		// TODO: zero-copy if supported. For selected threads only
	...
	// TODO: close, deinit, reinit, open if supported?

	printk("IPC-service HOST [INST %s - ENDP %s] demo ended.\n",
		       ept_data->label, ept_data->ept_cfg.name);
}
