...
static int cmd_net_udp_bind(const struct shell *shell, size_t argc, char *argv[])
{
#if !defined(CONFIG_NET_IPV6) || !defined(CONFIG_NET_UDP) || !defined(CONFIG_NET_NATIVE_UDP)
	ARG_UNUSED(shell);
	ARG_UNUSED(argc);
	ARG_UNUSED(argv);
    ...
    // TODO: Verify if context is used
...
static int cmd_net_udp_close(const struct shell *shell, size_t argc, char *argv[])
{
#if !defined(CONFIG_NET_IPV6) || !defined(CONFIG_NET_UDP) || !defined(CONFIG_NET_NATIVE_UDP)
	ARG_UNUSED(shell);
	ARG_UNUSED(argc);
    ...
    // TODO: Verify that context is used
...
static int cmd_net_udp_send(const struct shell *shell, size_t argc, char *argv[])
{
#if !defined(CONFIG_NET_IPV6) || !defined(CONFIG_NET_UDP) || !defined(CONFIG_NET_NATIVE_UDP)
	ARG_UNUSED(shell);
	ARG_UNUSED(argc);
    ...
    ret = net_context_sendto(udp_ctx, payload, strlen(payload), &addr, addrlen, udp_sent, K_FOREVER, NULL);
    if (ret < 0) {
